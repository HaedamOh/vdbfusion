...
from datasets import oxford_spires as Dataset
...
def main(
    ouster_scans: str,
    sequence: int = 0,
    config: str = "config/oxford_spires.yaml",
    n_scans: int = -1,
    jump: int = 0,
    ...
    pipeline = Pipeline(dataset, config, jump=jump, n_scans=n_scans, map_name="oxford_spires")
    pipeline.run()
